...
(use-package rg :defer t
  ...
  (when (sqlite-available-p)
    ...
    (defvar grep-analysis-history '() "Grep history")
    (defun grep-analysis-create-db ()
      (when memdb (sqlite-close))
      (setq memdb (sqlite-open))
      (sqlite-execute memdb "create table search (id integer primary key autoincrement, title text not null unique, idx integer default 0);")
      (sqlite-execute memdb "create table result (id integer primary key autoincrement, code text not null unique);")
      (sqlite-execute memdb "create table search_result (sid integer, rid integer, judge text not null default \"\", reason text not null default \"\", foreign key(sid) references search(id), foreign key(rid) references result(id));")
      )
    (defun grep-analysis-search-insert (title &optional idx)
      (if-let (sid (car (grep-analysis-search-find title)))
          sid
        (let ((cnt (sqlite-execute memdb "insert into search(title, idx) values (?,?)" (list title (or idx 0))))
              )
          (if (> cnt 0)
    ...
    (defun grep-analysis-search-find (title)
      (if-let (ret (sqlite-select memdb "select id, idx from search where title=?" (list title)))
          (car ret)
        nil
        )
      )
    (defun grep-analysis-search-update (title idx)
      (sqlite-execute memdb "update search set idx=? where title=?" (list idx title))
      )
    ...
    (defun grep-analysis-search-unbind-titles ()
      (if-let (ret (sqlite-select memdb "select title from search where idx=0"))
          (flatten-list ret)
        nil
        )
      )
    (defun grep-analysis-search-summary-levels (curlevel search searchs)
      (cond ((> (nth 4 search) 0) (list (list (nth 1 search) -1 -1 curlevel)))
            ((= (nth 2 search) 0) (list (list (nth 1 search) 0 0 curlevel)))
            (t (let ((subtitles (flatten-list (sqlite-select memdb "select judge from search_result where (judge not in (\"OK\", \"NG\", \"Ignore\", \"\")) and sid=?" (list (car search)))))
                     (out (list (list (nth 1 search) (nth 2 search) (nth 3 search) curlevel)))
                     )
                 (setf (nth 4 search) 1)
    ...
    (defun grep-analysis-search-summary ()
      (let* ((searchs (sqlite-select memdb "select s.id, s.title, (select count(*) from search_result sr where sr.sid = s.id) as total,  (select count(*) from search_result sr2 where sr2.sid = s.id and sr2.judge!=\"\") as part, 0 as used from search s group by s.id;"))
             (out '())
    ...
    (defun grep-analysis-lisp-find ()
      (let* ((cur-search (format "%s" rg-cur-search))
             (item (seq-find (lambda(elt) (string= cur-search (format "%s" (cdr elt)))) grep-analysis-history nil))
             )
        (if item (car item) nil)
        )
      )
    (defun grep-analysis-lisp-attach ()
      (unless memdb (grep-analysis-create-db))
      (if-let (title (grep-analysis-lisp-find))
          (car (grep-analysis-search-find title))
        (let* ((titles (grep-analysis-search-all-titles))
               (titles (push (rg-search-pattern rg-cur-search) titles))
          ...
          (unless (and sitem (> (nth 1 sitem) 0))
            (setq grep-analysis-history (push (cons newtitle (rg-search-copy rg-cur-search)) grep-analysis-history)
                  idx (length grep-analysis-history))
            (if sitem
                (progn
                  (setq sid (car sitem))
                  (grep-analysis-search-update newtitle idx)
                  )
              (setq sid (grep-analysis-search-insert newtitle idx))
              )
            (save-excursion
              (let ((not-moved 0)
                    )
    ...
    (defun thuleqaid/analysis-filter(buffer _)
      (when-let (title (grep-analysis-lisp-find))
    ...
    (defun thuleqaid/analysis-list()
      ...
      (let* ((searchs (grep-analysis-search-summary))
        ...
        (dotimes (idx searchcnt)
          ...
          (if (>= (nth 1 search) 0)
              (setq out (concat out "\n" (format (format "%% %dd. %%s%%s[%%d/%%d]" idx-width) idx (make-string (nth 3 search) 32) (car search) (nth 2 search) (nth 1 search))))
            (setq out (concat out "\n" (format (format "%% %dd. %%s%%s" idx-width) idx (make-string (nth 3 search) 32) (car search))))
            )
          )
        (setq out (concat out "\n\nRe-run grep:"))
        (let ((choice (read-number out))
          ...
          (when (< choice searchcnt)
            (setq title (car (nth choice searchs))
                  search (seq-find (lambda(elt) (string= title (car elt))) grep-analysis-history nil)
                  )
            (if search
                (progn
                  (setq rg-cur-search (rg-search-copy (cdr search)))
                  (rg-rerun t)
                  )
              (progn
                (kill-new title)
